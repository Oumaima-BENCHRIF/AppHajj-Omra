...
<br>
        <div class="flex justify-center">
            <a title="gestion Agent" class="intro-y w-10 h-10 rounded-full btn bg-slate-100 dark:bg-darkmode-400 dark:border-darkmode-400 text-slate-500 mx-2" href="{{ url('Agents') }}">1</button>
            <a  title="listes Agent" class="intro-y w-10 h-10 rounded-full btn btn-primary mx-2" href="{{ url('liste_Agents') }}">2</a>
    ...
    <h2 class="intro-y text-lg font-medium mt-10">Liste Agents</h2>
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
            ...
            <div class="hidden md:block mx-auto text-slate-500"> Affichage (Total lignes:{{$countListeAgents}})</div>
        ...
        <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
            <table class="table table-report -mt-2">
                <thead>
                    <tr>
                        <th class="text-center whitespace-nowrap">Code agents</th>
                        <th class="text-center whitespace-nowrap">Nom agents</th>
                ...
                <tbody>
                @foreach($listes_Agents as $key => $listesAgents)                       
                 <tr class="intro-x">
                            <td>
                                <div class="text-slate-500 text-xs whitespace-nowrap mt-0.5">{{$listesAgents->code_agents }}</div>
                            </td>
                            <td>
                                <div class="text-slate-500 text-xs whitespace-nowrap mt-0.5">{{$listesAgents->nom_agents }}</div>
                            </td>
                            <td>
                                <div class="text-slate-500 text-xs whitespace-nowrap mt-0.5">{{$listesAgents->telephone }}</div>
                            </td>
                            <td>
                                <div class="text-slate-500 text-xs whitespace-nowrap mt-0.5">{{$listesAgents->fax }}</div>
                            </td>
                            <td>
                                <div class="text-slate-500 text-xs whitespace-nowrap mt-0.5">{{$listesAgents->adresse }}</div>
                            </td>
                            <?php $url_info = route('hotel_transports.infos'); ?>
                            <td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    <!-- update -->
                                    <button onclick="showDialogueModifierAccompagnateurs('{{ $listesAgents->id }}')" href="javascript:;" data-tw-toggle="modal" data-tw-target="#header-footer-modal-preview" class="flex items-center mr-3">
                                    ...
                                    <!-- delete -->
                                    <button onclick="showDialogueDeleteAgents('{{ $listesAgents->id }}')" value="{{ $listesAgents->id }}" class="flex items-center text-danger" href="javascript:;" data-tw-toggle="modal" data-tw-target="#delete-confirmation-modal">
                                                ...
                                                {{ csrf_field() }}
                                                    <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                                                        <div class="col-span-12 sm:col-span-6">
                                                            <label for="code_agents" class="form-label">Code agent</label>
                                                            <input id="code_agents" name="code_agents" value="{{old('code_agents')}}" type="text" class="form-control" placeholder="Code">
                                                        </div>

                                                        <input type="hidden" id="id_" name="id_" >

                                                        <div class="col-span-12 sm:col-span-6">
                                                            <label for="nom_agents" class="form-label">Nom & agent</label>
                                                            <input id="nom_agents" name="nom_agents"  value="{{old('nom_agents')}}" type="text" class="form-control" placeholder="Nom & prenom">
                                                        </div>
