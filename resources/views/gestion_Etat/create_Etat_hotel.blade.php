    ...
    <div id="example-tab-3" class="tab-pane leading-relaxed active" role="tabpanel" aria-labelledby="example-3-tab">
        <div class="tab-content intro-y box py-5 px-5  mt-5" id="example-tab-3" class="tab-pane leading-relaxed active" role="tabpanel" aria-labelledby="example-3-tab">
            ...
            <div class="px-5 mt-10 mb-6">
                <div class="font-medium text-center text-lg">Gestion état par Hôtel et par Pax</div>
            </div>
            <form  id="search_allotement" action="{{route('Etat_Hotel.list_Allotement')}}" method="get">
                {{ csrf_field() }}
                <div class="py-5 px-2  container">
                    <div class="form-inline">
                    <div class="intro-y w3 px-1  @if ($errors->get('hotel')) has-error @endif">
                            <label class="form-label mbt-2 text-size">hotel</label>
                            <select id="hotel" name="hotel" data-search="true" class="form-control py-1">
                            </select>
                            @if($errors->get('hotel'))
                            @foreach($errors->get('hotel') as $message)
                            <li class="text-danger">{{$message}}</li>
                            @endforeach
                            @endif
                        </div>

                        <div class="intro-y w3 px-1  @if ($errors->get('code_agents')) has-error @endif">
                        ...
                        <div class="intro-y w3 px-1  @if ($errors->get('date_reteur')) has-error @endif">
...
<script type="text/javascript" src="{{URL::asset('js/Gestion_EtatHotel.js')}}"></script>
